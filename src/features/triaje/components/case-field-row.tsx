...
interface CaseFieldRowProps {
   ...
   * revisor sepa que el match se está consultando.
   */
      ...
      {!isBool && (
        ...
        <Controller
          ...
          render={({ field: rhf }) => {
            ...
            const isBeneficiaryDni = field.id === 'beneficiary.dni';
            const input = (
              <Input
                ...
                className={cn(
                  ...
                  isBeneficiaryDni && isMdmMatching && 'pr-32',
            ...
            // La fila del DNI del beneficiario muestra un spinner "Buscando en
            // MDM…" dentro del campo mientras se verifica (debounce de 350ms +
            // fetch) si el DNI ya está registrado en el maestro. Así el revisor
            // sabe que el match se está consultando en lugar de un delay
            // silencioso.
            if (!isBeneficiaryDni || !isMdmMatching) return input;

            return (
              <div className="relative">
                {input}
                <span className="pointer-events-none absolute inset-y-0 right-2 flex items-center gap-1.5">
                  <Loader2 className="size-3 animate-spin text-info-dark" />
                  <span className="font-data text-[10px] font-medium whitespace-nowrap text-info-dark">
                    Buscando en MDM…
                  </span>
                </span>
              </div>
