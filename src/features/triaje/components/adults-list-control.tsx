...
export function AdultsListControl() {
  ...
  watchedAdults.forEach((adult, idx) => {
    ...
    const label =
      adult.full_name ||
      getRelationshipLabel(adult.relationship);
            ...
            <div className="flex items-center justify-between border-b border-border/50 pb-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  // Adjust guardian_ref and emergency_contact_ref when indices shift
                  // after removal. Must use setValue (not _defaultValues) so that
                  // RHF reactive state updates and the UI re-renders correctly.
                  const gRef = parseInt(guardianRef, 10);
                  if (gRef === index) {
                    setValue('guardian_ref', '');
                  } else if (gRef > index) {
                    setValue('guardian_ref', String(gRef - 1));
                  }
                  
                  const eRef = parseInt(emergencyRef, 10);
                  if (eRef === index) {
                    setValue('emergency_contact_ref', '');
                  } else if (eRef > index) {
                    setValue('emergency_contact_ref', String(eRef - 1));
                  }
                }}
                      ...
                      <SelectTrigger className="h-8 font-data text-[12.5px] shadow-sm focus:ring-primary/20">
                        <SelectValue placeholder="Seleccionar...">
                          {(() => {
                            ...
                            return val ? getRelationshipLabel(val) : "Seleccionar...";
                          })()}
                        </SelectValue>
                      </SelectTrigger>
                      <SelectContent>
